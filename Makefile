...
EXTENSION_NAME=SANEQL
EXTENSION_FLAGS=\
-DDUCKDB_EXTENSION_NAMES="saneql" \
...
#### Main build
# For regular CLI build, we link the saneql extension directly into the DuckDB executable
CLIENT_FLAGS=-DDUCKDB_EXTENSION_${EXTENSION_NAME}_SHOULD_LINK=1

debug:
	mkdir -p  build/debug && \
	cmake $(GENERATOR) $(CMAKE_FLAGS) $(BUILD_FLAGS) $(CLIENT_FLAGS) -DCMAKE_BUILD_TYPE=Debug -S ./duckdb/ -B build/debug && \
	cmake --build build/debug --config Debug --parallel

release:
	mkdir -p build/release && \
	cmake $(GENERATOR) $(CMAKE_FLAGS) $(BUILD_FLAGS)  $(CLIENT_FLAGS)  -DCMAKE_BUILD_TYPE=Release -S ./duckdb/ -B build/release && \
...
#### Client tests
DEBUG_EXT_PATH='$(PROJ_DIR)build/debug/extension/saneql/saneql.duckdb_extension'
RELEASE_EXT_PATH='$(PROJ_DIR)build/release/extension/saneql/saneql.duckdb_extension'
